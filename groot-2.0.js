            ...
            (function () {
                ...
                function bindSinle(vm, pro, ve) {
                    ...
                    if (!record["_elUi1" + pro]) {
                        _elUi = getElement(_elUi0, PREFIX + "-widget", pro);
                        if (_elUi.length == 0) {
                    ...
                    if (!record["_elUi1" + pro]) {
                        _elUi.each(function () {
                            var _widgetname = $(this).attr(PREFIX + "-widget");
                            _widgetname = _widgetname.split(":")[1];
                            $(this).removeAttr(PREFIX + "-widget");
                            var _data = null;
                            if (typeof $(this).attr(PREFIX + "-widget-data") != "undefined") {
                                var _default = $(this).attr(PREFIX + "-widget-data");
                                if (_default.indexOf("widgetInit") > -1) {
                                    _default = _default.substring(_default.indexOf("[") + 1, _default.lastIndexOf("]"));
                                    _data = groot.widgetInit[_default];
                                } else {
                                    _data = eval("(" + _default + ")");
                                }

                                $(this).removeAttr(PREFIX + "-widget-data");
                            }
                            var _id = getRandom();
                            if (typeof $(this).attr(PREFIX + "-widget-id") != "undefined") {
                                _id = $(this).attr(PREFIX + "-widget-id");
                                $(this).removeAttr(PREFIX + "-widget-id");
                            } else {
                                _id = "widget" + _id;
                            }
                            _widgetList.push(_id);
                            groot.widget[_widgetname]($(this), _id, _data, vm[pro], (function (_id) {
                                return function () {
                                    vm[pro] = groot.vms[_id].widgetvalue;
                                    vm[pro + "Render"]();
                                }
                            })(_id));
                        });
                    ...
                    vm[pro + RENDER] = function () {
                        ...
                        for (var i = 0; i < _widgetList.length; i++) {
                            groot.vms[_widgetList[i]].widgetvalue = value;
                            groot.vms[_widgetList[i]].widgetvalueRender();
                        }
                        /*********************** 触发监控  *******************************/
                ...
                function getElement(eles, selector, value) {
                    var retlist = [];
                    eles.each(function () {
                        if ($(this).attr(selector) === value || (selector == PREFIX + "-widget" && $(this).attr(selector) !== undefined && $(this).attr(selector).indexOf(value + ":") === 0)) {
                            retlist.push(this);
                        }
                    })
                    return $(retlist);
...
//---------------groot API---------------//
    groot.UIComponent = function (config) {
        var url = config.url;
        var id = config.id;
        var data = config.data;
        var uuid = getRandom();
        return require(url).UIComponent($("#" + id)[0], uuid, data);
    }
